...
from .datasets import json_dataset, split_ds, ConcatDataset, SplitDataset, bert_sentencepair_dataset, GPT2Dataset
...
def make_dataset(path, enc_seq_length, dec_seq_length, text_key, label_key, lazy=False, process_fn=None, split=[1.],
    ...
    if should_split(split):
        ...
        if ds_type.lower() == 'bert':
            ...
            ds = [bert_sentencepair_dataset(d, max_seq_len=enc_seq_length, presplit_sentences=presplit_sentences)  if d is not None else None  for d in ds]
        elif ds_type.lower() == 'gpt2':
            ds = [GPT2Dataset(d, max_seq_len=dec_seq_length) if d is not None else None for d in ds]
        else:
            assert 0, "No such dataset"
    else:
        if ds_type.lower() == 'bert':
            presplit_sentences = kwargs['presplit_sentences'] if 'presplit_sentences' in kwargs else False
            ds = bert_sentencepair_dataset(ds, max_seq_len=enc_seq_length, presplit_sentences=presplit_sentences)
        elif ds_type.lower() == 'gpt2':
            ds = GPT2Dataset(ds, max_seq_len=dec_seq_length)
        else:
            assert 0, "No such dataset"
