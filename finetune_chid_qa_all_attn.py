...
from model import AllAttnGPT2Model
...
class CHIDDataset(torch.utils.data.Dataset):
    ...
    def process(self, data):
        ...
        for d in tqdm(data[:int(self.ratio * len(data))]):
            ...
            # assert len(d["cand_poses"]) == 10, len(d["cand_poses"])
            # assert max(d["cand_poses"]) < len(d["sent"]), print(d["cand_poses"], len(d["sent"]))
            # assert d["mask_pos"] < len(d["sent"]), print(d["mask_pos"], len(d["sent"]))
            # assert d["sent_pos"] < len(d["sent"]), print(d["sent_pos"], len(d["sent"]))
    ...
    def collate(self, samples):
        ...
        batch_seq = {
            ...
            "attention_mask": torch.zeros((bs, 1, max_size, max_size)),
            "position_ids": position_ids,
            "cand_poses": torch.zeros(bs, 10).long(),
            "mask_pos": torch.zeros(bs).long(),
            "sent_pos": torch.zeros(bs).long()
        }
        ...
        for i, samp in enumerate(seq):
            ...
            batch_seq["attention_mask"][i, 0, :len(samp["input_ids"]), :len(samp["input_ids"])] = torch.ones(len(samp["input_ids"]), len(samp["input_ids"]))
        
        for i, samp in enumerate(ints):
            no_model_seq["truth"][i] = torch.tensor(samp["truth"])
            batch_seq["cand_poses"][i] = torch.tensor(samp["cand_poses"])
            batch_seq["mask_pos"][i] = samp["mask_pos"]
            batch_seq["sent_pos"][i] = samp["sent_pos"]

        if self.args.fp16:
            batch_seq["attention_mask"] = batch_seq["attention_mask"].half()
...
def main():
    ...
    # load data
    train_dataloader, _ = load_data('/data/gyx/chid/preprocessed_qa_all_attn', 'train', tokenizer, 1)
    dev_dataloader, dev_dataset = load_data('/data/gyx/chid/preprocessed_qa_all_attn', 'dev', tokenizer, 1)
    ...
    model, optimizer, lr_scheduler = setup_model_and_optimizer(args, model_cls=AllAttnGPT2Model)
    ...
    for e in range(epoch):
        ...
        for batch, no_model_batch in tqdm(train_dataloader, disable=torch.distributed.get_rank() != 0):
            ...
            for k in no_model_batch:
                ...
                # if torch.distributed.get_rank() == 0:
                #     print(k)
                #     print(no_model_batch[k][0])

            # exit(0)
            output = model(**batch)
            losses = F.cross_entropy(output, no_model_batch["truth"])
            ...
            if torch.distributed.get_rank() == 0:
                scores = torch.stack(tensor_list, 0).view(-1, 10) # for convience, the truth labels only appear in the first part of the model
