...
class CHIDDataset(torch.utils.data.Dataset):
    def __init__(self, args, data_path, split, tokenizer, idiom_score, ratio=1):
        self.split = split
...
class IdiomDataset(torch.utils.data.Dataset):
    def __init__(self, args, data_path, tokenizer):
        ...
        self.seq, self.sizes = self.process(data)
    ...
    def process(self, data):
        sizes = []
        seq = []
        for iid, idiom in enumerate(data):
            input_ids = self.tokenizer.encode(idiom)
            length = len(input_ids) - 1
            sizes.append(length)
            seq.append({
                "iid": iid,
    ...
    def collate(self, samples):
        ...
        for i, samp in enumerate(seq):
            ...
            no_model_seq["loss_mask"][i, :len(samp["loss_mask"])] = torch.tensor(samp["loss_mask"])
            no_model_seq["labels"][i, :len(samp["labels"])] = torch.tensor(samp["labels"])
            no_model_seq["iids"][i] = torch.tensor(samp["iid"])
...
def load_data(data_path, data_type, tokenizer, idiom_score, ratio=1):
    args = get_args()
    ...
    dataset = CHIDDataset(args, filename, data_type, tokenizer, idiom_score, ratio=ratio)
...
def load_idiom_data(data_path, tokenizer):
    args = get_args()
    batch_size = args.batch_size
    args.batch_size = 1
    ...
    # Dataset
    filename = os.path.join(data_path, "idioms.json")
    dataset = IdiomDataset(args, filename, tokenizer)
...
def get_idiom_score(tokenizer, data_path, model, device):
    file_path = os.path.join(data_path, "idiom_scores.pkl")
    ...
    idiom_dataloader, idiom_dataset = load_idiom_data(data_path, tokenizer)
    all_iids = []
    all_losses = []
    with torch.no_grad():
        for batch, no_model_batch in tqdm(idiom_dataloader, desc="Building Idioms"):
            for k in batch:
            ...
            iids = no_model_batch["iids"]
            iid_tensor_list = [torch.zeros_like(iids) for _ in range(mpu.get_data_parallel_world_size())]
            torch.distributed.all_gather(iid_tensor_list, iids.data, group=mpu.get_data_parallel_group())
            all_iids.extend(iid_tensor_list)

    all_iids = torch.stack(all_iids).view(-1).cpu().detach().numpy()
    all_losses = torch.stack(all_losses).view(-1).cpu().detach().numpy()

    all_scores = [0 for _ in all_iids]

    for iid, loss in zip(all_iids, all_losses):
        all_scores[iid] = loss
...
def main():
    ...
    data_path = '/data/gyx/chid/preprocessed_unbias/'
    ...
    for e in range(epoch):
        ...
        with torch.no_grad():
            for batch, no_model_batch in tqdm(dev_dataloader, desc="Evaluating"):
                ...
                loss = loss - args.alpha * no_model_batch["bias_score"]
